...
class Geometry_nonuniform(object):
    def __init__(self, config):
        self.v_res = np.array(config['nVoxel'])    # ct scan, in order of x,y,z
        self.p_res = np.array(config['nDetector']) # projections, not sure about the order
        self.v_spacing = np.array(config['dVoxel'])    # mm, in order of x,y,z
